...
public class AuthService {
    ...
    public ResponseDTO registerMember(MemberDTO memberDTO) {
        try {
            ...
            return new ResponseDTO(VarList.RSP_SUCCESS, "Registration successful", savedMemberDTO, token);
    ...
    public ResponseDTO login(LoginDTO loginDTO) {
        try {
            ...
            if (loginDTO.getRole() != null && loginDTO.getRole().equals("MEMBER")) {
                Member member = memberRepository.findByMemEmail(loginDTO.getEmail()).orElse(null);
                if (member != null) {
                    user = modelMapper.map(member, MemberDTO.class);
                }
            }
            // Check if it's an admin login
            else if (loginDTO.getRole() != null && loginDTO.getRole().equals("ADMIN")) {
                Admin admin = adminRepository.findByAdminEmail(loginDTO.getEmail()).orElse(null);
                if (admin != null) {
                    user = modelMapper.map(admin, AdminDTO.class);
                }
            }
